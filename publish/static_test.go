...
func TestStaticContactsEverySeedOnce(t *testing.T) {
	...
	// with every seed contacted and none having replied the publish is waiting
	require.IsType(t, &StatePublishWaiting{}, state)
}

// TestStaticWaitingReportsNothingDue checks that a static publish waiting on store record
...
func TestStaticWaitingReportsNothingDue(t *testing.T) {
	...
	state = sm.Advance(ctx, now, &EventPublishPoll{})
	require.IsType(t, &StatePublishWaiting{}, state)
	require.True(t, state.(*StatePublishWaiting).NextDue.IsZero())
}

// TestStaticFinishesWhenEveryStoreIsReported checks that a static publish reports the nodes
...
// TestStaticEmptySeedFinishesImmediately checks that a static publish started with no nodes
...
// TestStaticStopRecordsOutstandingNodesAsFailed checks that stopping a static publish
...
// TestStaticFinishedIsSticky checks that a static publish polled after it has finished
